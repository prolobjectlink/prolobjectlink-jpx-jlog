...
public final class JLogEngine extends JavaEngine implements PrologEngine {
    ...
    JLogEngine(PrologProvider provider) {
	super(provider);
    ...
    String toString(jPrologServices engine) {
    ...
    jRule toRule(String str, jPrologServices engine) {
	jKnowledgeBase kb = engine.getKnowledgeBase();
	pOperatorRegistry or = engine.getOperatorRegistry();
	pPredicateRegistry pr = engine.getPredicateRegistry();
	String clause = str.charAt(str.length() - 1) == '.' ? str : str + DOT;
	jTerm term = new pParseStream(clause, kb, pr, or).parseTerm();
	switch (term.type) {
    ...
    jRule toRule(PrologTerm head, PrologTerm... body) {
	...
	try {
		...
		while (r.hasMoreElements()) {
			...
			// rule head
			jPredicate ruleHead = jRule.getHead();
			writer.print(ruleHead.toString(true));
			...
			if (k.hasMoreElements()) {
			    ...
			    while (k.hasMoreElements()) {
				jTerm term = (jTerm) k.nextElement();
				writer.print(term.toString(true));
				if (k.hasMoreElements()) {
				    writer.print(COMMA);
				}
			    }
	...
	kb.getRuleDefinitionsMatch(na).clearRules();
    }
	...
	for (Enumeration<?> e = kb.enumDefinitions(); e.hasMoreElements();) {
		...
		for (Enumeration<?> r = rds.enumRules(); r.hasMoreElements();) {
			...
			// Stack<jVariable> stack = new Stack<jVariable>();
			// boolean headUnify = JLogTermUtil.unify(rh, head,
			// stack);
			// boolean bodyUnify = JLogTermUtil.unify(rb, body,
			// stack);
			// if (headUnify && bodyUnify) {
			// return true;
			// }
